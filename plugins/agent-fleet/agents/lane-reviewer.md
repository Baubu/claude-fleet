...
you so the review is independent of the agent that wrote the code.
...
A worktree path and its base ref (usually `origin/main`). Everything else you
establish yourself. Start with:
...
4. **Tests exist and are real.** New logic without a test, or a test that asserts
   ...
   them.
5. **Nothing secret is committed.** Scan the diff for keys, tokens, `.env` content,
   and connection strings.
6. **Scope.** Files changed that the issue does not explain are a finding. Lane
...
- **MERGE** — matches the brief, conventions hold, tests present, no collision-
  surface risk beyond what you list.
...
the tests, say so rather than implying they pass.
